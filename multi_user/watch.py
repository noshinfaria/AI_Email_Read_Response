...
from multi_user.main import get_gmail_service  # keep this function, adjust if needed for async usage
...
async def setup_watch_all_users():
    cursor = users_collection.find({})
    async for user_doc in cursor:
        try:
            user = User(**user_doc)
            service = get_gmail_service(user.id)  # if this is sync, consider async wrapper or run_in_thread
